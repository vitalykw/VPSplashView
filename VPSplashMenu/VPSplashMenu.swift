...
class VPSplashMenu : UIView{
    ...
    // Tint color of icons
    var secondColor : UIColor = UIColor.whiteColor(){
        didSet {
            self.setup()
        }
    }
    ...
    private var circles: [VPSplashCircle]!
    ...
    */
    ...
    private func setup() {
        ...
        self.mainCircle = VPSplashCircle(center:CGPointMake(self.bounds.size.width/2, self.bounds.size.height/2), radius: self.circlesRadius, color: self.mainColor)
    ...
    func setupItems() -> [VPSplashCircle] {
        return Array (0..<self.numberOfMenuItems()).map { i in
            ...
            if (dataSource == nil){
                ...
                button.icon = dataSource?.menuIconForIndexPath(NSIndexPath(forRow: i, inSection: 0))
            }
    ...
    func handleTap(touchPoint: CGPoint) {
        if (shown == true){
            self.circles?.forEach({
                let expectedFrame = CGRectMake($0.expectedCenter.x-$0.bounds.width/2, $0.expectedCenter.y-$0.bounds.height/2, $0.bounds.width, $0.bounds.height)
    ...
    func cancelTap(){
        self.circles?.forEach({
            if ($0.animated){
                self.delegate?.pickedItemAtIndex(NSIndexPath(forRow: $0.tag, inSection: 0))
            }
        })
    }
    ...
    func connectionLayerWith(circle:VPSplashCircle)-> CAShapeLayer{
    ...
    func squash(){
        ...
        if (shown == false){
            ...
            self.bringSubviewToFront(self.mainCircle)
            self.circles?.forEach({
                $0.animateMove()
                $0.unhighlight()
            })
    ...
    func movedTo(centralPoint:CGPoint){
